...
/**
 * Item-Auswahl speziell für das Beantworten einer Anfrage.
 * 
...
public class ResponseItemSelectionGui extends AbstractWagerGui {
    ...
    public ResponseItemSelectionGui(EventPlugin plugin, Player player, WagerSession session, CommandRequest request) {
    ...
    public void open() {
        inventory = Bukkit.createInventory(null, SIZE,
            MessageUtil.color(t("response-item-selection.title")));
    ...
    private void buildLayout() {
        ...
        inventory.setItem(CLEAR_ALL_SLOT, createButton(Material.BARRIER,
            t("response-item-selection.clear-title"),
            t("response-item-selection.clear-line1"),
            t("response-item-selection.clear-line2")));
        ...
        // Reihen 3-5: Spieler-Inventar Kopie
        displayPlayerInventory();
    }
    
    private ItemStack createInfoItem() {
        return createButton(Material.NETHER_STAR, t("response-item-selection.info-title"),
            "",
            t("response-item-selection.info-line1"),
            t("response-item-selection.info-line2"),
            "",
            t("response-item-selection.info-line3"),
            t("response-item-selection.info-line4"),
            "",
            t("response-item-selection.info-line5"),
            t("response-item-selection.info-line6"));
    }
    ...
    private void displaySelectedItems() {
        ...
        for (int i = SELECTED_START; i <= SELECTED_END; i++) {
            inventory.setItem(i, createButton(Material.LIME_STAINED_GLASS_PANE,
                t("response-item-selection.empty-slot-title"),
                t("response-item-selection.empty-slot-lore")));
        }
    ...
    private void addRemoveLore(ItemStack item) {
        ...
        if (meta != null) {
            ...
            lore.add("");
            lore.add(MessageUtil.color(t("response-item-selection.remove-lore")));
            meta.setLore(lore);
    ...
    private void addClickToAddLore(ItemStack item) {
        ...
        if (meta != null) {
            List<String> lore = meta.hasLore() ? new ArrayList<>(meta.getLore()) : new ArrayList<>();
            lore.add("");
            lore.add(MessageUtil.color(t("response-item-selection.add-lore")));
            meta.setLore(lore);
            item.setItemMeta(meta);
        }
    }
    ...
    private void handleSelectedItemClick(int slot) {
    ...
    private int determineAmount(ClickType clickType, int maxAmount) {
        switch (clickType) {
            case LEFT: return 1;
            case RIGHT: return maxAmount;
            case SHIFT_LEFT:
            case SHIFT_RIGHT: return Math.max(1, maxAmount / 2);
            default: return 1;
        }
    }
    ...
    private void addItemToWagerFromSlot(int playerSlot, int amount) {
        ...
        if (session.getWagerItemCount() >= 7) {
            playErrorSound();
            MessageUtil.sendMessage(player, t("response-item-selection.error-max-items"));
            return;
        }
    ...
    private void handleClearAll() {
        ...
        playClickSound();
        MessageUtil.sendMessage(player, t("response-item-selection.cleared-message"));
        refreshDisplay();
    }
