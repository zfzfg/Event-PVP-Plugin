...
public class ItemSelectionGui extends AbstractWagerGui {
    ...
    public void open() {
        inventory = Bukkit.createInventory(null, SIZE, 
            de.zfzfg.core.util.Text.of(t("items-title")));
    ...
    private void buildLayout() {
        ...
        inventory.setItem(CLEAR_ALL_SLOT, createButton(Material.BARRIER, 
            t("item-selection.clear-title"),
            t("item-selection.clear-line1"),
            t("item-selection.clear-line2")));
    ...
    private ItemStack createInfoItem() {
        ...
        lore.add("");
        lore.add(MessageUtil.color(t("item-selection.info-line1")));
        lore.add(MessageUtil.color(t("item-selection.info-line2")));
        lore.add("");
        lore.add(MessageUtil.color(t("item-selection.info-line3")));
        lore.add(MessageUtil.color(t("item-selection.info-line4")));
        lore.add("");
        lore.add(MessageUtil.color(t("item-selection.info-line5")));
        lore.add(MessageUtil.color(t("item-selection.info-line6")));
        
        return createButton(Material.NETHER_STAR, t("item-selection.info-title"), lore);
    }
    ...
    private void displaySelectedItems() {
        ...
        for (int i = SELECTED_START; i <= SELECTED_END; i++) {
            inventory.setItem(i, createButton(Material.LIME_STAINED_GLASS_PANE,
                t("item-selection.empty-slot-title"),
                t("item-selection.empty-slot-lore")));
        }
    ...
    private void addRemoveLore(ItemStack item) {
        ...
        if (meta != null) {
            ...
            lore.add(de.zfzfg.core.util.Text.ofItem(t("item-selection.remove-lore")));
            meta.lore(lore);
    ...
    private void addClickToAddLore(ItemStack item) {
        ...
        if (meta != null) {
            List<net.kyori.adventure.text.Component> lore = meta.hasLore() ? new ArrayList<>(meta.lore()) : new ArrayList<>();
            lore.add(net.kyori.adventure.text.Component.empty());
            lore.add(de.zfzfg.core.util.Text.ofItem(t("item-selection.add-lore")));
            meta.lore(lore);
            item.setItemMeta(meta);
        }
    }
    ...
    private void addItemToWagerFromSlot(int playerSlot, int amount) {
        ...
        if (session.getWagerItemCount() >= 7) {
            playErrorSound();
            MessageUtil.sendMessage(player, MessageUtil.color(t("item-selection.error-max-items")));
            return;
        }
    ...
    private void handleClearAll() {
        ...
        playClickSound();
        MessageUtil.sendMessage(player, MessageUtil.color(t("item-selection.cleared-message")));
        refreshDisplay();
    }
