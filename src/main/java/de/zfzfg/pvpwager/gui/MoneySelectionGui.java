...
public class MoneySelectionGui extends AbstractWagerGui {
    ...
    private void buildLayout() {
        ...
        inventory.setItem(CLEAR_SLOT, createButton(Material.BARRIER,
            t("money-selection.clear-title"),
            t("money-selection.clear-line")));
    ...
    private ItemStack createInfoItem() {
        ...
        lore.add("");
        lore.add(MessageUtil.color(t("money-selection.choose-line1")));
        lore.add(MessageUtil.color(t("money-selection.choose-line2")));
        
        return createButton(Material.GOLD_BLOCK, t("money-title"), lore);
    }
    ...
    private void createAmountButtons() {
        for (int i = 0; i < AMOUNT_SLOTS.length && i < AMOUNTS.length; i++) {
            ...
            lore.add("");
            if (canAfford) {
                lore.add(MessageUtil.color(t("money-selection.amount-select-line1")));
                lore.add(MessageUtil.color(t("money-selection.amount-select-line2")));
            } else {
                lore.add(MessageUtil.color(t("money-selection.amount-not-enough-money")));
            }
    ...
    private void updateCurrentAmount() {
        ...
        List<String> lore = new ArrayList<>();
        lore.add("");
        lore.add(MessageUtil.color(t("money-selection.current-label")));
        lore.add(MessageUtil.color("&6&l$" + String.format("%,.2f", current)));
        lore.add("");
        lore.add(MessageUtil.color(t("money-selection.remaining-balance")));
        ...
        inventory.setItem(CURRENT_SLOT, createButton(material,
            t("money-selection.current-title"), lore));
    }
