...
public class ResponseMoneySelectionGui extends AbstractWagerGui {
    ...
    private void buildLayout() {
        ...
        inventory.setItem(CLEAR_SLOT, createButton(Material.BARRIER,
            t("response-money-selection.clear-title"),
            t("response-money-selection.clear-lore")));
    ...
    private ItemStack createInfoItem() {
        ...
        lore.add("");
        lore.add(MessageUtil.color(t("response-money-selection.info-balance")));
        lore.add(MessageUtil.color("&6$" + String.format("%,.2f", balance)));
        lore.add("");
        lore.add(MessageUtil.color(t("response-money-selection.info-opponent")));
        lore.add(MessageUtil.color("&6$" + String.format("%,.2f", senderMoney)));
        
        return createButton(Material.GOLD_BLOCK, t("response-money-selection.info-title"), lore);
    }
    ...
    private void createAmountButtons() {
        for (int i = 0; i < AMOUNT_SLOTS.length && i < AMOUNTS.length; i++) {
            ...
            lore.add("");
            if (!canAfford) {
                lore.add(MessageUtil.color(t("response-money-selection.not-enough-money")));
            }
    ...
    private void createMatchSenderButton() {
        ...
        inventory.setItem(MATCH_SENDER_SLOT, createButton(Material.SUNFLOWER,
            t("response-money-selection.match-sender-title"),
            "",
            t("response-money-selection.match-sender-line1") + " &6$" + String.format("%,.2f", senderMoney),
            "",
            t("response-money-selection.match-sender-line2"),
            t("response-money-selection.match-sender-line3")));
    }
    ...
    private void updateCurrentAmount() {
        ...
        List<String> lore = new ArrayList<>();
        lore.add("");
        lore.add(MessageUtil.color(t("response-money-selection.current-label")));
        lore.add(MessageUtil.color("&6&l$" + String.format("%,.2f", current)));
        lore.add("");
        lore.add(MessageUtil.color(t("response-money-selection.remaining-balance")));
        ...
        inventory.setItem(CURRENT_SLOT, createButton(material, t("response-money-selection.current-title"), lore));
    }
