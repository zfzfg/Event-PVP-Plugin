...
public class DrawSubCommand extends SubCommand {
    ...
    // Store scheduled tasks to cancel them when draw is accepted
    private final Map<UUID, BukkitTask> drawVoteTasks = new ConcurrentHashMap<>();
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length > 0) {
            if (args[0].equalsIgnoreCase("accept")) {
                ...
                BukkitTask task = drawVoteTasks.remove(match.getMatchId());
                if (task != null) {
                    task.cancel();
                }
                // Deactivate the draw vote
        ...
        BukkitTask task = plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            if (match.cancelDrawVoteIfInitiator(player.getUniqueId())) {
                drawVoteTasks.remove(match.getMatchId());
                // Only show expired message if draw vote is still active (not already accepted)
                if (match.isDrawVoteActive()) {
                    broadcastLines(match, cfg.getMessage("messages.command.draw.expired"));
                }
            }
        }, Time.seconds(drawVoteTime));
        drawVoteTasks.put(match.getMatchId(), task);
