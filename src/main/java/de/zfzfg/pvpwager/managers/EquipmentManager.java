...
public class EquipmentManager {
        ...
        if (unifiedSection != null) {
            for (String setId : unifiedSection.getKeys(false)) {
                try {
                    ...
                    plugin.getLogger().log(java.util.logging.Level.SEVERE, "Error loading equipment '" + setId + "'", e);
                }
            }
        }
        ...
        if (equipmentSets.isEmpty()) {
            ...
            for (String setId : setsSection.getKeys(false)) {
                ...
                plugin.getLogger().info("Loaded legacy equipment set: " + setId + " (" + displayName + ")");
                    ...
                    plugin.getLogger().log(java.util.logging.Level.SEVERE, "Error loading equipment set '" + setId + "'", e);
                }
            }
        }
    ...
    private ItemStack parseItem(String materialName) {
        ...
        String cacheKey = materialName.toUpperCase();
        Material material = materialCache.get(cacheKey);
        
        if (material == null) {
            try {
                material = Material.valueOf(cacheKey);
    ...
    private void applyEnchantments(ItemStack item, List<String> enchantments) {
        for (String enchantString : enchantments) {
            try {
                ...
                if (enchantment == null) {
                    enchantment = Registry.ENCHANTMENT.get(NamespacedKey.minecraft(enchantName.toLowerCase()));
                    if (enchantment != null) {
                        enchantmentCache.put(enchantName, enchantment);
                    }
                }
    ...
    private void applyAllowedWorldsFromSection(EquipmentSet equipmentSet, ConfigurationSection setSection) {
        try {
            ...
            plugin.getLogger().warning("Failed to apply allowed worlds from section: " + e.getMessage());
        }
    }
}