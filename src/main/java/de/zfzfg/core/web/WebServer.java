...
public class WebServer {
    ...
    private final Map<String, java.util.concurrent.atomic.AtomicInteger> rateLimitCounters = new ConcurrentHashMap<>();
    ...
    private boolean checkRateLimit(HttpExchange exchange) {
        String clientIp = exchange.getRemoteAddress().getAddress().getHostAddress();
        java.util.concurrent.atomic.AtomicInteger counter = rateLimitCounters.computeIfAbsent(clientIp, k -> new java.util.concurrent.atomic.AtomicInteger(0));
        if (counter.incrementAndGet() > 100) { // 100 requests per window
            sendJsonResponse(exchange, 429, Map.of("success", false, "error", "Rate limit exceeded"));
            return false;
        }
        return true;
    }
