...
public class InventorySnapshotStorage {
    ...
    private static String extractLastFourDigits(String s) {
        ...
        try {
            ...
            // Silently ignore regex errors - return null as fallback
        }
        return null;
    }
    ...
    public static void pruneOldEntries(EventPlugin plugin) {
        pruneSingleFile(plugin, FILE_NAME, "Snapshot-Datei");
        pruneSingleFile(plugin, FILE_NAME_POST, "Post-Snapshot-Datei");
    }

    private static void pruneSingleFile(EventPlugin plugin, String fileName, String label) {
        try {
            File file = new File(plugin.getDataFolder(), fileName);
            if (!file.exists()) return;
            YamlConfiguration cfg = YamlConfiguration.loadConfiguration(file);
            ...
            org.bukkit.configuration.ConfigurationSection entriesSection = cfg.getConfigurationSection("entries");
            if (entriesSection != null) {
                Set<String> uuids = entriesSection.getKeys(false);
                for (String uuidKey : uuids) {
                    List<Map<?, ?>> rawList = cfg.getMapList("entries." + uuidKey);
            ...
            // Delete file on disk if empty after prune
            try {
                boolean empty = true;
                org.bukkit.configuration.ConfigurationSection sec = cfg.getConfigurationSection("entries");
                if (sec != null) {
                    for (String uuidKey : sec.getKeys(false)) {
                        java.util.List<java.util.Map<?, ?>> rawList = cfg.getMapList("entries." + uuidKey);
                        if (rawList != null && !rawList.isEmpty()) { empty = false; break; }
                    }
                }
                if (empty && file.exists() && !file.delete()) {
                    plugin.getLogger().warning("Konnte leere " + label + " nicht löschen: " + file.getName());
                }
            } catch (Exception ignored) {}
        } catch (IOException e) {
            plugin.getLogger().warning("Failed to prune " + label + ": " + e.getMessage());
        } catch (Exception e) {
            plugin.getLogger().warning("Unexpected error pruning " + label + ": " + e.getMessage());
        }
    }
