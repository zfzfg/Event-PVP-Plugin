...
public class ArenaSelectionGui extends AbstractWagerGui {
    ...
    private ItemStack createInfoItem() {
        ...
        lore.add("");
        lore.add(MessageUtil.color(t("available-arenas-count", "count", String.valueOf(availableArenas.size()))));
        lore.add("");
        ...
        return createButton(Material.GRASS_BLOCK, t("arena-title"), lore);
    }
    ...
    private void displayArenas() {
        ...
        if (availableArenas.isEmpty()) {
            inventory.setItem(22, createButton(Material.BARRIER,
                t("no-arenas-available-title"),
                t("no-arenas-available-lore")));
        }
    }
    ...
    private ItemStack createArenaItem(Arena arena) {
        ...
        List<String> lore = new ArrayList<>();
        lore.add("");
        lore.add(MessageUtil.color(t("world-label", "world", arena.getArenaWorld())));
        lore.add(MessageUtil.color(t("spawn-type-label", "type", arena.getSpawnType().name())));
        
        if (arena.getBoundaries() != null) {
            lore.add(MessageUtil.color(t("boundaries-active")));
        }
        ...
        return createButton(material, t("arena-item-title", "name", prefix + arena.getDisplayName()), lore);
    }
    ...
    public boolean handleClick(InventoryClickEvent event) {
        ...
        switch (slot) {
            ...
            case CONFIRM_SLOT:
                if (session.getSelectedArena() != null) {
                    ...
                    playErrorSound();
                    MessageUtil.sendMessage(player, MessageUtil.color(t("error-select-arena")));
                }
                return true;
        }
        ...
        if (index >= 0 && index < availableArenas.size()) {
            ...
            playClickSound();
            MessageUtil.sendMessage(player, MessageUtil.color(t("arena-selected-message", "arena", selected.getDisplayName())));
